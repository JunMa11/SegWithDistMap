...
parser.add_argument('--model', type=str,  default='vnet_dp_la_AAAISDFL1', help='model_name')
parser.add_argument('--gpu', type=str,  default='0', help='GPU to use')
parser.add_argument('--epoch_num', type=int,  default='6000', help='model to use')
...
def dist_test_all_case(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None):
    total_metric = 0.0
...
def test_single_case(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                out_dist = net(test_patch)
                y = torch.sigmoid(-1500*out_dist)
                # print(y.shape, out_dist.shape) # ([1, 1, 112, 112, 80]) ([1, 1, 112, 112, 80])
                # pdb.set_trace() 
                y = y.cpu().data.numpy()
                y = y[0,:,:,:,:]
    ...
    label_map = (score_map>0.5).astype(score_map.dtype).squeeze()
    if add_pad:
